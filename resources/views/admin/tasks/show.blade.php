...
@section('content')
    <div class="card stat-card p-4 mb-3">
        ...
        <p class="text-muted small">Due: {{ $task->due_date?->toDateString() ?? '—' }} {{ $task->due_time }}</p>
        <div class="mb-2">{{ $task->description ?: '—' }}</div>
        <div class="mb-2"><strong>Assigned to:</strong> {{ $task->assignees->pluck('name')->join(', ') ?: '—' }}</div>
            ...
            <div class="card stat-card p-4">
                ...
                @forelse ($task->reports as $report)
                    <div class="border rounded p-3 mb-2">
                        ...
                        <a href="{{ route('admin.task-reports.show', $report) }}" class="btn btn-sm btn-outline-primary">Review</a>
                    </div>
                @empty
                    <p class="text-muted small mb-0">No reports submitted yet.</p>
                @endforelse
            </div>
        </div>
        <div class="col-md-5">
